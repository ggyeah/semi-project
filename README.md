...
### 팀원 송현정
- **HOME화면 구현**
  - `for each`문과 `HashMap` 기능을 이용하여 DB에 등록된 상품의 사진, 상품명, 가격, 판매상태 등이 최신등록순으로 12개씩 노출되도록 구현
  - 조건문을 활용하여 할인이 적용 중인 상품인 경우 **할인된 가격**이 노출되도록 구현
- **상품 관리**
  - 관리자2(최고위직)의 경우에만 상품 추가/수정/삭제할 수 있도록 권한 차등 부여
  - 상품이미지를 **multipart**형식의 form으로 File타입의 객체를 받아와서 DB와 로컬폴더에 **파일이 저장**되도록 구현
  - 상품이미지가 있는 제품만 조회할 수 있도록 구현
  - 상품 구매 완료 시 **상품재고량** 및 **판매상태** 변경 기능 구현
  - 상품 추가 및 수정 입력폼에 `jQuery`를 적용하여 **유효성 검사 실시**
  - 상품 삭제 시 `AJAX`를 적용하여 **비동기 처리**
  - 상품별 **리뷰 개수** 조회 기능 구현
  - 상품 이름 **검색기능** 및 **검색결과 개수** 표시 기능 구현
- **카테고리 관리**
  - **카테고리별 상품** 조회 기능 및 **조회결과 개수** 표시 기능 구현
  - 카테고리 추가 및 수정 입력폼에 `jQuery`를 적용하여 **유효성 검사 실시**
  - 카테고리 삭제 시 `AJAX`를 적용하여 **비동기 처리**
### 팀원 임지예
