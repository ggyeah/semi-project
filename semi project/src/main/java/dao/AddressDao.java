...
public class AddressDao {
	...
	public ArrayList<Address> selectAddress(String loginId) throws Exception {
		...
		String selectAddressSql = "select address_no, id, address_name, address, address_last_date, default_address, createdate, updatedate from address WHERE id = ? ORDER BY updatedate";
	...
	// 2) 배송지 추가
		public int addAddress(Address address) throws Exception {
			DBUtil dbUtil = new DBUtil();
			Connection conn = dbUtil.getConnection();
			String addAddressSql = "INSERT INTO address(address_name, address, address_last_date, default_address, createdate, updatedate) values(?, ?, ?, ?, NOW(), NOW())";
			PreparedStatement addAddressStmt = conn.prepareStatement(addAddressSql);
			addAddressStmt.setString(1, address.getAddressName());
			addAddressStmt.setString(2, address.getAddress());
			addAddressStmt.setString(3, address.getAddressLastDate());
